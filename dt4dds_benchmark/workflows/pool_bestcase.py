...
class Pool_Bestcase(BaseWorkflow):
    """
    Pool workflow assuming synthesis by material deposition methods and amplification by a high-fidelity polymerase.

    After initialization, the workflow can be used to run a simulation by calling run().    
    """
    coverage: float = 50

    command_path = str(pathlib.Path(__file__).parent.absolute() / 'bin' / 'workflow_pool_bestcase' / 'run.sh')
